 ...
 *  revise AM 1804 to be instantiable class for comm to Roomba
 -- main Proc .pde sketch has user I/O methods and instances this inner
...
{  // declare, init misc. variables, constants
  ...
  /** current speed for movements that don't get speed param */
  ...
  /** Time in milliseconds between Auto sensor updates, not sensors()  */
  int sensorsUpdateTime = 300;
  ...
  /*  overloaded send fx's
   * first sends a byte array to Roomba 
   ...
   * hooked to the serial adapter's DTR line, which may not be 
  ...
  public void control() { 
    logmsg("control");
    mode = MODE_SAFE; // probably not there yet
    send( CONTROL );
  ...
  public void reset() {
    ...
    pause(300);
    //  test if this work
    if (sensorsValid) println("OK, we're back on");
  }
 ...
 * Spin in place clockwise, at the input speed.
  ...
  public void drive( int velocity, int radius ) {
    byte cmd[] = { 
      (byte)DRIVE, (byte)(velocity>>>8), (byte)(velocity&0xff), 
